...
function Module_Field(){
	...
	$this->style = $webdb[Info_style]?$webdb[Info_style]:'default';
...
function formGetVale($field_db,&$rsdb,$CKfield=''){
	foreach($field_db AS $key=>$rs){
		...
		if($rs[form_type]=='select'){
			...
			$rsdb[$key]=En_TruePath($rsdb[$key],0);
			$rsdb[$key]=str_replace(array("'","<",">"),array("&#39;","&lt;","&gt;"),$rsdb[$key]);
		}elseif($rs[form_type]=='classdb'){
...
function make_template($id){
	...
	$indexshow_tpl = $tplDB->read_tpl("bencandy_{$_type}.htm",$tpldb[indexshow],'index');
	$indexpost_tpl = $tplDB->read_tpl("post_{$_type}.htm",$tpldb[indexpost],'index');
	$indexsearch_tpl = $tplDB->read_tpl("search_{$_type}.htm",$tpldb[indexsearch],'index');	
	$indexlist_tpl = $tplDB->read_tpl("list_{$_type}.htm",$tpldb[indexlist],'index');
