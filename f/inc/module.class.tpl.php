...
function post_tpl($rs,&$tplcode){
	...
	{
		$show0="{$rs[title]}:$mustfill<br>{$rs[form_title]}";
		$show="<input type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='50' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]<br><iframe frameborder=0 height=23 scrolling=no src='../upfile.php?fn=upfile&dir=\$_pre/\$fid&label=atc_{$rs[field_name]}' width=310></iframe>";
	}
	//单文件上传
...
</script><textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:99%;height:200px;visibility:hidden;'>\$rsdb[{$rs[field_name]}]</textarea>";
	}
