...
if($job=="list")
{
	...
	$rows=20;
	$min=($page-1)*$rows;
	$showpage=getpage("{$_pre}dianping","","?job=$job","$rows");
	...
	while($rs=$db->fetch_array($query)){
		...
		if($rs[yz]==1){
			$rs[yz]="<A HREF='?action=list&jobs=unyz&ciddb[{$rs[cid]}]=$rs[cid]' style='color:blue;'>已审核</A>";
		}elseif($rs[yz]==0){
			$rs[yz]="<A HREF='?action=list&jobs=yz&ciddb[{$rs[cid]}]=$rs[cid]' style='color:red;'>未审核</A>";
		}
		$listdb[]=$rs;
	}
	require("head.php");
	require("template/dianping/list.htm");
	require("foot.php");
}
elseif($action=="list")
{
...
elseif($job=="show")
{
	...
	require("head.php");
	require("template/dianping/show.htm");
	require("foot.php");
}