...
if($job=='list')
{
	require(Adminpath."../php168/module_db.php");
	require("head.php");
	require("template/replace/list.htm");
	require("foot.php");
}
elseif($job=="choose")
{
	...
	@extract( $db->get_one("SELECT config FROM {$_pre}module WHERE id='$mid'") );
	$m_config=unserialize( $config );
	...
	require("head.php");
	require("template/replace/choose.htm");
	require("foot.php");
}
elseif($action=='replace')
{
	...
	{
		$page++;
		echo "请稍候,正在处理当中$page<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?action=$action&page=$page'>";
		exit;
	}
	unlink(Adminpath."../cache/module_db.php");
	refreshto("replace.php?job=list","处理完毕",10);
}