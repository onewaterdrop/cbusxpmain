...
if($job=='out'){
	...
	if(file_exists(Adminpath."../cache/bak_mysql.txt"))
	{
		$breakbak=read_file(Adminpath."../cache/bak_mysql.txt");
	}
...
elseif($job=='into'){
	$selectname=bak_time();
	if(file_exists(Adminpath."../cache/mysql_insert.txt")){
		echo "<CENTER><table><tr bgcolor=#FF0000><td colspan=5 height=30><div align=center><A HREF=".read_file(Adminpath."../cache/mysql_insert.txt")."><b><font color=ffffff>上次还原数据被中断是否继续,点击继续</font></b></A></div></td></tr></table></CENTER>";
	}
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/mysql/menu.htm");
...
function bak_out($tabledb){
	...
	if($tabledb[$tableid])
	{
		...
		//记录下来.防止中途备份失败
		write_file(Adminpath."../cache/bak_mysql.txt","?lfj=$lfj&action=out&page=$page&rowsnum=$rowsnum&tableid=$tableid&rand_dir=$rand_dir&step=$step&tabledbreto=$Table&baksize=$baksize&baktype=$baktype");
...
EOT;
	...
	else
	{
		...
		@unlink(Adminpath."../cache/bak_mysql.txt");
		$rs[totalsize]=$totalsize;
		$rs[timedir]=$rand_dir;
		if( !@is_writable(Adminpath."../cache/mysql_bak/$rand_dir/0.sql") ){
			showerr("备份失败，请在分类目录的cache/mysql_bak/目录下创建一个目录mysql然后改其属性为0777,如果此目录已存在，请删除他，重新创建，并改属性为0777");
		}
		return $rs;
	}
}
...
function bak_into(){
	...
	$step++;
	if( file_exists(Adminpath."../cache/mysql_bak/$baktime/{$step}.sql") ){
		write_file(Adminpath."../cache/mysql_insert.txt","?lfj=mysql&action=into&baktime=$baktime&step=$step");
		echo "已导入第 {$step} 卷<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?lfj=mysql&action=into&baktime=$baktime&step=$step'>";
		exit;
	}else{

		@unlink(Adminpath."../cache/mysql_insert.txt");
