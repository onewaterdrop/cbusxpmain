...
$linkdb=array(
			...
			"回收站的信息"=>"?job=list&type=del&fid=$fid",
...
if($job=="list")
{
	...
	$RS['FOUND_ROWS()'] && $showpage=getpage(" "," ","?job=list&fid=$fid&type=$type&keyword=".urlencode($keyword),$rows,$RS['FOUND_ROWS()']);
	while($rs=$db->fetch_array($query))
	{
		...
		if(!$rs[yz]){
			$rs[ischeck]="<A HREF='?action=work&id=$rs[id]&jobs=yz' style='color:black;'>未审核</A>";
		}elseif( $rs[yz]==1){
			$rs[ischeck]="<A HREF='?action=work&id=$rs[id]&jobs=unyz' style='color:blue;'>已审核</A>";
		}elseif( $rs[yz]==2 ){
			$rs[ischeck]="<A HREF='?action=work&id=$rs[id]&jobs=undel' style='color:blue;'>作废</A>";
		}
		if(!$rs[levels]){
			$rs[iscom]="<A HREF='?action=work&id=$rs[id]&jobs=com&levels=1' style=''>未推荐</A>";
		}else{
			$rs[iscom]="<A HREF='?action=work&id=$rs[id]&jobs=com&levels=0' style='color:red;'>已推荐</A>";
		}
	...
	require("head.php");
	require("template/post/list.htm");
	require("foot.php");
}
elseif($job=="work")
{
	...
	require("head.php");
	require("template/post/work.htm");
	require("foot.php");
}
elseif($action=="work")
{
