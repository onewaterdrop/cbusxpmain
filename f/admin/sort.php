...
$linkdb=array(
			  "栏目管理"=>"sort.php?job=listsort","批量操作"=>"?job=batch"
			);

if($fid){
	$linkdb["修改栏目"]="sort.php?job=editsort&fid=$fid";
	$linkdb["全部字段"]="?job=listfield&fid=$fid";
	$linkdb["增加字段"]="?job=addfield&fid=$fid";
}
...
if($job=="listsort")
{
	...
	if($webdb[Info_sys]){
		...
		$labelUrl="../list.php?choose_cityID=$cityid&jobs=show";
	}
...
{
	require("head.php");
	require("template/sort/batch.htm");
	require("foot.php");
}
elseif($action=="batch")
{
...
elseif($job=='table')
{
	...
	require("head.php");
	require("template/sort/table.htm");
	require("foot.php");
}
elseif($action=='table')
{
	...
	if($ckk){
		$page++;
		write_file("../cache/table.txt","sort.php?action=$action&tableid=$tableid&tableid0=$tableid0&fid=$fid&page=$page");
		echo "请稍候...$page<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?action=$action&tableid=$tableid&tableid0=$tableid0&fid=$fid&page=$page'>";
		exit;
	}else{
		...
		refreshto("sort.php?job=listsort","操作成功","1");
	}
}
