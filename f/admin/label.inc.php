...
if($action=='mod'){
	...
	require(dirname(__FILE__)."/../php168/all_fid.php");
	...
	$SQL=implode(" UNION ALL ",$sql_DB)." ORDER BY $order $asc LIMIT $rows ";
	...
	if($yz=='ad'){
		...
		$SQL="SELECT C.*,D.* FROM {$_pre}buyad A LEFT JOIN {$_pre}db D ON A.id=D.id LEFT JOIN {$_pre}content C  ON D.id=C.id WHERE A.sortid=-1 AND A.endtime>\$GLOBALS[timestamp] $SQL LIMIT $rows";
	}
	...
	$select_city=select_city('cityId',$cityId);
...
function select_city($name,$fid=''){
	global $db,$_pre;
	$ck=$fid=='$GLOBALS[city_id]'?'selected':'';
	$show.="<select name='$name'><option value=''>所有城市</option><option value='\$GLOBALS[city_id]' $ck>对应城市</option>";
