...
$linkdb["模块管理"]="?job=listsort";
if($id){
	$linkdb["全部字段"]="module.php?job=editsort&id=$id";
	$linkdb["增加字段"]="module.php?job=addfield&id=$id";
	$linkdb["生成模块"]="module.php?job=tpl&id=$id";
}
...
if($job=="listsort")
{
	...
	$showpage=getpage("{$_pre}module A","$SQL","?job=$job&sort_id=$sort_id","$rows");
	...
	$selectsortid.="</select>";
	require("head.php");
	require("template/module/sort.htm");
	require("foot.php");
}
...
elseif($action=="addsort")
{
	...
	write_module_cache();
	refreshto("?job=editsort&id=$id","创建成功");
}
...
elseif($job=="editsort")
{
	...
	require("head.php");
	require("template/module/editsort.htm");
	require("foot.php");
}
...
elseif($action=="editorder")
{
	...
	write_module_cache();
	$Module_db->make_template($id);
	refreshto("?job=editsort&id=$id","修改成功",10);
}
elseif($job=="editfield")
{
	...
	require("head.php");
	require("template/module/editfield.htm");
	require("foot.php");
}
elseif($action=="editfield")
{
	...
	refreshto("?job=editsort&id=$id","修改成功",1);
}
elseif($job=="addfield")
{
	$rsdb = $Module_db->module_info($id);
...
elseif($action=="addfield")
{
	...
	write_module_cache();
	refreshto("?job=editsort&id=$id",'创建成功',10);
}
...
elseif($job=='tpl')
{
	if($automaketpl){	//批量生成模板
		...
		$id=$rsdb[id];	
		if(!$id){
			refreshto("module.php?job=listsort","模板生成完毕",3);
		}else{
			$Module_db->make_template($id);
			$page++;
			echo "正在生成模板:$rsdb[name]<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?job=$job&automaketpl=$automaketpl&page=$page'>";
			exit;
		}	
	}else{
		$Module_db->make_template($id);
		refreshto("module.php?job=listsort","模板生成完毕",1);
	}
...
elseif($job=="editmodule")
{
	...
	require("head.php");
	require("template/module/editmodule.htm");
	require("foot.php");
}
...
elseif($job=="setcomment")
{
	...
	require("head.php");
	require("template/module/setcomment.htm");
	require("foot.php");
}
...
function write_module_cache(){
	...
	write_file("../php168/module_db.php",$show.'?>');
}
?>