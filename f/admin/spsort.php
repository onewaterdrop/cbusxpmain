...
$linkdb=array("地域/省份管理"=>"?job=listsort","城市管理"=>"?job=city","城市辖区管理"=>"?job=zone","地段管理"=>"?job=street","批量操作"=>"?job=batch");
...
if($job=="listsort")
{
	...
	$fid=intval($fid);
	$sortdb=array();
	list_allsort(0,$table='area');
	$sort_fup=$Guidedb->Select("{$_pre}area","fup",$fid);

	require("head.php");
	require("template/spsort/sort.htm");
	require("foot.php");
}
...
elseif($job=="city")
{
	...
	require("head.php");
	require("template/spsort/city.htm");
	require("foot.php");
}
...
elseif($job=="zone")
{
	...
	$rows=100;
	$min=($page-1)*$rows;
	$showpage=getpage("{$_pre}zone","$SQL","?job=$job&fup=$fup",$rows);
	...
	require("head.php");
	require("template/spsort/zone.htm");
	require("foot.php");
}
elseif($action=="addstreet")
{
	...
	if(!$rs){
		refreshto("?job=zone","请选择一个城市辖区",3);
	}
...
elseif($job=="street")
{
	...
	$rows=100;
	$min=($page-1)*$rows;
	$showpage=getpage("{$_pre}street","$SQL","?job=$job&fup=$fup",$rows);
	...
	require("head.php");
	require("template/spsort/street.htm");
	require("foot.php");
}
//修改栏目信息
elseif($job=="editsort")
{
	...
	$sort_fid=$Guidedb->Select("{$_pre}area","postdb[fid]",$fid,"?job=$job");
	$sort_fup=$Guidedb->Select("{$_pre}area","postdb[fup]",$rsdb[fup]);

 	$typedb[$rsdb[type]]=" checked ";

	require("head.php");
	require("template/spsort/editsort.htm");
	require("foot.php");
}
elseif($job=="edit_city")
{
	...
	require("head.php");
	require("template/spsort/edit_city.htm");
	require("foot.php");
}
elseif($job=="edit_zone")
{
	...
	require("head.php");
	require("template/spsort/edit_zone.htm");
	require("foot.php");
}
...
elseif($job=="edit_street")
{
	...
	require("head.php");
	require("template/spsort/edit_street.htm");
	require("foot.php");
}
...
elseif($action=="edit_city")
{
	...
	if($postdb[head]&&!is_file("../$postdb[head]")){
		showerr("头部文件不存在");
	}elseif($postdb[foot]&&!is_file("../$postdb[foot]")){
		showerr("尾部文件不存在");
	}elseif($postdb[index]&&!is_file("../$postdb[index]")){
		showerr("主页文件不存在");
	}
...
{
	require("head.php");
	require("template/spsort/batch.htm");
	require("foot.php");
}
elseif($action=="batch")
{
	...
	if($type=="zone_dir"){
		...
		if($page<2){
			$db->query("UPDATE {$_pre}city SET dirname=''");
		}
		$rows=5;
		$page<1 && $page=1;
		$min=($page-1)*$rows;
		...
		if($ckk){
			$page++;
			echo "请稍候$page....<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?action=$action&type=$type&domain2=$domain2&citydir=$citydir&page=$page'>";
			exit;
		}else{
			area_cache_guide();
		}		
	}
	refreshto("?job=$action","操作成功",1);
}
/**
*更新缓存
**/
function area_cache_guide(){
	...
	write_file("../php168/all_area.php",$show);
	city_cache_guide();
}
...
function city_cache_guide(){
	...
	$dir=opendir("../php168/zone/");
	while($f=readdir($dir)){
		if(eregi(".php$",$f)){
			unlink("../php168/zone/$f");
		}
	}
	$show="<?php\r\nunset(\$city_DB);";
	...
	while($rs = $db->fetch_array($query)){
		...
		if($rs['dirname']&&is_dir("../$rs[dirname]")){
			...
			if($rs[domain]){
				...
				$conf.='
<VirtualHost *:80>
DocumentRoot '.dirname(dirname(__FILE__))."/$rs[dirname]".'
...
';
			...
			elseif(eregi("^\.\.",$rs['dirname'])){
				$_url="\$city_DB[url][{$rs[fid]}]=\$webdb[www_url].'/".str_replace("../","",$rs['dirname'])."/';";
			}else{
				$_url="\$city_DB[url][{$rs[fid]}]=\$Mdomain.'/$rs[dirname]/';";
			}
		}else{
			if($webdb[Info_sys]){
				$_url="\$city_DB[url][{$rs[fid]}]=\$webdb[www_url].'/index.php?choose_cityID=$rs[fid]';";
			}else{
				$_url="\$city_DB[url][{$rs[fid]}]=\$Mdomain.'/index.php?choose_cityID=$rs[fid]';";
			}			
		}
...
";
	...
	write_file("../php168/all_city.php",$show.'?>');
	$conf && write_file("apache.txt",$conf);
}
function zone_cache_guide($fup){
	...
	if(!is_dir("../php168/zone/")){
		mkdir("../php168/zone/");
		chmod("../php168/zone/",0777);
	}
	write_file("../php168/zone/$fup.php",$show.'?>');
}
...
function get_area_guide($cityid=0,$zoneid=0){
	...
	if($zoneid){
		...
		$rs=$db->get_one($SQL);
		$show="<A HREF='?job=zone&fup=$rs[cityid]'>$rs[city]</A> -> <A HREF='?job=street&fup=$rs[zoneid]'>$rs[zone]</A>";
	}elseif($cityid){
		...
		$rs=$db->get_one($SQL);
		$show="<A HREF='?job=city&fup=$rs[proid]'>$rs[pro]</A> -> <A HREF='?job=zone&fup=$rs[cityid]'>$rs[city]</A>";
	}
	return $show;
}
...
function write_city_file($dirname,$cityid){
	...
	makepath(PHP168_PATH.Mdirname."/".$dirname);
	if(!is_dir(PHP168_PATH.Mdirname."/".$dirname)){
		showerr(PHP168_PATH.Mdirname."/".$dirname."目录创建失败,请确认分类目录权限可写");
	}
	if( eregi("^\.\.",$dirname) ){
	...
	write_file(PHP168_PATH.Mdirname."/$dirname/index.php",$string);
	write_file(PHP168_PATH.Mdirname."/$dirname/list.php",$string);
	write_file(PHP168_PATH.Mdirname."/$dirname/bencandy.php",$string);
	write_file(PHP168_PATH.Mdirname."/$dirname/post.php",$string);
	write_file(PHP168_PATH.Mdirname."/$dirname/job.php",$string);
	write_file(PHP168_PATH.Mdirname."/$dirname/search.php",$string);	
}
?>