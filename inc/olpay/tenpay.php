<?php
if(!function_exists('html') && $_GET[signMsg]){
...
class tenpay_config
{
	...
	//以下每一项都必须要配置，并准确
	var $spid 				="1210215301";			//卖家帐号
	var $sp_key				="cdc2627881c7f97dc4416dd25a19b485";																		//密钥
	var $domain				="http://www.or58.com"	;	//商户网站域名
	...
	var $return_url			='http://www.or58.com';
	...
	function tenpay_interface_pay ($bank_type,$desc,$purchaser_id,$sp_billno,$total_fee,$attach,$ip)
	{
		...
		if ($retcode < "09000")//判断是否为严重错误，>09000为严重错误
		{
			if ($beta_switch == "1") //判断测试开关，如果开启测试，支付金额为1分 
			{
				...
				$sign_text ="cmdno=1" . "&date=" . date('Ymd') . "&bargainor_id=" . $this->tenpay_config->spid ."&transaction_id=" . $this->tenpay_config->spid . date('Ymd').time()."&sp_billno=" . $sp_billno . "&total_fee=" . $total_fee . "&fee_type=1"  . "&return_url=" . $this->tenpay_config->return_url . "&attach=" . $attach."&spbill_create_ip=".$spbill_create_ip ;
...
if($_GET[signMsg]){
	...
	$strResponseText  = "cmdno=" . $strCmdno . "&pay_result=" . $strPayResult . 
							  "&date=" . $strBillDate . "&transaction_id=" . $strTransactionId .
								...
								"&fee_type=" . $strFeeType . "&attach=" . $strAttach ."&spbill_create_ip=".$spbill_create_ip.
...
else
{
	...
	$tenpay_conf->return_url="$webdb[www_url]/inc/olpay/tenpay.php?".str_replace(array("=","&"),array("--","---"),$array[return_url]);
