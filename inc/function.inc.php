...
function En_TruePath($content,$type=1,$ifgetplayer=0){
	...
	if($type==1)
	{
		...
		if($webdb[mirror]){
			$content=str_replace("$webdb[mirror]","http://www_or58_com/Tmp_updir",$content);
		}
		$content=str_replace("$webdb[www_url]/$webdb[updir]","http://www_or58_com/Tmp_updir",$content);		
		$content=str_replace("$webdb[www_url]","http://www_or58_com",$content);
	}
	else
	{
		//使用了远程附件镜像,要做特别处理,不局限于使用FTP
		if($webdb[mirror]){
			$content=preg_replace("/http:\/\/www_or58_com\/Tmp_updir\/([-_=\/\.A-Za-z0-9]+)/eis","tempdir('\\1')",$content);
		}else{
			$content=str_replace("http://www_or58_com/Tmp_updir","$webdb[www_url]/$webdb[updir]",$content);
		}		
		$content=str_replace("http://www_or58_com","$webdb[www_url]",$content);
		if($ifgetplayer){
...
function ipfrom($ip) {
	...
	if( !is_file(PHP168_PATH.'inc/ip.dat') ){
		return '<a title><A HREF="http://down2.or58.com/ip.rar" title="点击下载后,解压放到整站/inc/目录即可">IP库不存在,请点击下载一个!</A></a>';
	}
...
function check_imgnum($yzimg){
	...
	$webdb[yzimg_letter_differen] || $yzimg=strtolower($yzimg);
	$time=$timestamp-1800;	//半小时前的验证码失效.
	if($db->get_one("SELECT * FROM {$pre}yzimg WHERE BINARY imgnum='$yzimg'")){
