...
function get_module_outpic($str,$getpic=1){
	...
	foreach( $listdb AS $key=>$value){
		...
		if($ck){
			$str=str_replace("$value","http://www_or58_com/Tmp_updir/form/$name",$str);
		}
	}
	return $str;
}
...
function get_outpic($str,$fid=0,$getpic=1){
	...
	foreach( $listdb AS $key=>$value){
		...
		if($ck){
			$str=str_replace("$value","http://www_or58_com/Tmp_updir/article/$fid/$name",$str);
		}
	}
	return $str;
}
...
function post_new(){
	global $db,$pre,$Fid_db,$postdb,$fid,$fidDB,$mid,$lfjuid,$lfjdb,$webdb,$timestamp,$FROMURL,$aid,$votesdb,$vote_db,$groupdb,$web_admin;
...
function post_edit(){
	global $db,$pre,$postdb,$fid,$fidDB,$Fid_db,$mid,$lfjuid,$rsdb,$lfjdb,$webdb,$timestamp,$aid,$FROMURL,$groupdb,$web_admin;
