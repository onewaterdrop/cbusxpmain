...
class search
{
	    ...
	    while($row=$db->fetch_array($query))
         {
			  ...
			  $data["txt"]=$this->GetRedKeyWord(str_cut($row["description"],200));
			  $data["url"]=$row["url"];
   ...
   function GetTg()
   {
		...
		$sql="select a.*,b.* from ve123_zz_links a left join ve123_zz_user b on a.user_id=b.user_id where b.points>=a.price and a.user_id>0 and ".$keywordsql." order by price desc limit 20";//echo $sql;
	    $query=$db->query($sql);
	    while($row=$db->fetch_array($query))
         {
		      $data["title"]=$this->GetRedKeyWord(str_cut($row["title"],60));
