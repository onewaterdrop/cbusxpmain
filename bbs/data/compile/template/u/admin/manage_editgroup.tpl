...
<div class="wrap">
	<div class="nav">
		...
		<ul class="cc">
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?uid=', rawurlencode($uid),'&m=u&c=manage&a=edit'; ?>">编辑</a></li>
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?uid=', rawurlencode($uid),'&m=u&c=manage&a=editCredit'; ?>">积分</a></li>
			<li class="current"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?uid=', rawurlencode($uid),'&m=u&c=manage&a=editGroup'; ?>">用户组</a></li>
		</ul>
	</div>
<!--====================用户编辑开始====================-->
<!--用户组-->
<div class="h_a">编辑用户组</div>
<form class="J_ajaxForm" action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=u&c=manage&a=doEditGroup'; ?>" method="post">
		...
		<tr>
			...
			<td>
				<select id="J_u_group_select" class="select_5" name="groupid">
					...
					<?php  } ?>
					<option value="<?php echo htmlspecialchars($item['gid'], ENT_QUOTES, 'UTF-8');?>"<?php echo htmlspecialchars(Pw::isSelected($info['groupid'] == 0), ENT_QUOTES, 'UTF-8');?>>普通组</option>
				</select>
			</td>
