...
<script src="<?php echo htmlspecialchars(Wind::getComponent('response')->getData('G','url','js'), ENT_QUOTES, 'UTF-8');?>/wind.js?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>"></script>
	...
	<link href="<?php echo Wekit::app()->themes.'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/profile.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
	...
	<div class="main_wrap">
		<div class="main cc">
			<div class="bread_crumb">
				<a href="<?php echo Wekit::app()->baseUrl; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=profile'; ?>">设置</a>
<?php 
...
?>
				<em>&gt;</em><a href="<?php echo htmlspecialchars($_item['url'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($_item['title'], ENT_QUOTES, 'UTF-8');?></a>
<?php }?>
			</div>
...
?>
		<li class="<?php echo htmlspecialchars($_item['current'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo WindUrlHelper::createUrl($_item['url'], array('_left' => $key));?>" id="profile_<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($_item['title'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php } else { ?>
		<li class="<?php echo htmlspecialchars($_item['current'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=profile&c=extends&_left=', rawurlencode($key); ?>" id="profile_<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($_item['title'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php 
	}
}?>
	</ul>
...
<!--头像上传开始-->
							<div class="content">
								<div class="profile_nav">
									<ul>
										<li class="current"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=profile&c=avatar&_left=avatar'; ?>">更改头像</a></li>
									</ul>
								</div>
							<?php if ($isAvatarBan) {
							...
							<?php } elseif ($type != 'nomal') {?>
								<div class="profile_ct">
										...
										<div class="other_upload">如果无法上传头像，请尝试使用<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=profile&c=avatar&type=nomal'; ?>">普通上传模式</a></div>
										<script>
							...
							<?php } else {?>
								<div class="avatar_other">
									...
									<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
									<div class="other_upload">返回 <a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=profile&c=avatar'; ?>">高级上传模式</a></div>
								</div>
							<?php }?>
							</div>
<!--头像上传结束-->
...
Wind.ready(document, function(){
	Wind.use('jquery', 'global', GV.JS_ROOT + 'pages/profile/profileAvatarNormal.js?v=' + GV.JS_VERSION);
});
</script>
</body>
</html>
