...
<link href="<?php echo Wekit::app()->themes.'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/tag.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
</head>
<body>
...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<a href="<?php echo Wekit::app()->baseUrl; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag'; ?>">话题</a><em>&gt;</em><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($tag['tag_name']); ?>"><?php echo htmlspecialchars($tag['tag_name'], ENT_QUOTES, 'UTF-8');?></a>
		</div>
			...
			<div class="main_body">
				<div class="main_content">
					<div class="box_wrap tag_info_box cc">
						<div class="banner"><img src="<?php echo htmlspecialchars(Pw::getPath($tag['tag_logo']), ENT_QUOTES, 'UTF-8');?>" width="110" alt="<?php echo htmlspecialchars($tag['tag_name'], ENT_QUOTES, 'UTF-8');?>" onerror="this.onerror=null;this.src='<?php echo Wekit::app()->themes.'/site/default/images'; ?>/tags/default.png'"/></div>
						<div class="name"><h3><?php echo htmlspecialchars($tag['tag_name'], ENT_QUOTES, 'UTF-8');?></h3>
						<?php  if (!$tag['attention']) { ?>
						<a rel="nofollow" id="J_follow_btn" data-type="add" data-id="<?php echo htmlspecialchars($tag['tag_id'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&id=', rawurlencode($tag['tag_id']),'&type=add'; ?>" class="core_follow J_qlogin_trigger">关注该话题</a>
						<?php  } else { ?>
						<a rel="nofollow" id="J_follow_btn" data-type="del" data-id="<?php echo htmlspecialchars($tag['tag_id'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&id=', rawurlencode($tag['tag_id']),'&type=del'; ?>" class="core_unfollow J_qlogin_trigger">取消关注</a>
						<?php  } ?>
						</div>
						<div class="num">
							<ul class="cc">
					...
					<div class="box_wrap">
						<nav>
							<div class="content_nav">
								...
								?>
									<li <?php echo $thread_current;?>><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($tag['tag_name']); ?>">最新帖子</a></li>
									<li <?php echo $user_current;?>><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($tag['tag_name']),'&type=users'; ?>">会员</a></li>
								</ul>
							</div>
						</nav>
						<div class="tag_page">
						...
						<div class="tag_uesr_list">
							<ul class="cc">
								...
								<li>
									...
									<p><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($u['uid']); ?>" class="J_user_card_show" data-uid="<?php echo htmlspecialchars($u['uid'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($u['username'], ENT_QUOTES, 'UTF-8');?></a></p>
								</li>
								<?php  } ?>
							</ul>
						</div>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&page=', rawurlencode($_page_i); 
...
<?php } ?>
						...
						<div class="not_content">
							<p>啊哦，暂没有会员关注该话题哦，我来<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&id=', rawurlencode($tag['tag_id']),'&type=add'; ?>" id="J_none_follow" class="J_qlogin_trigger">关注一下</a>！</p>
						</div>
						<?php  } 
  } else { 
  if ($contents) { ?>
						<div class="tag_view_list">
							<div class="feed_lists" id="J_tag_feed_lists">
								...
								?>
								<dl class="cc">
									<dt class="feed_face"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>" class="J_user_card_show" data-uid="<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($value['created_userid'], 'small'), ENT_QUOTES, 'UTF-8');?>" data-type="small" width="50" height="50" alt="<?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?>" /></a></dt>
									<dd class="feed_content">
										<?php  if ($allowManage) { 
  if ($value['tagifcheck']) { ?>
										<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=doshield&id=', rawurlencode($tag['tag_id']),'&type_id=', rawurlencode($value['type_id']),'&param_id=', rawurlencode($value['param_id']),'&ifcheck=0'; ?>" class="shield J_tag_shield" title="将该帖移出这个话题">屏蔽</a>
											<?php  } else { ?>
										<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=doshield&id=', rawurlencode($tag['tag_id']),'&type_id=', rawurlencode($value['type_id']),'&param_id=', rawurlencode($value['param_id']),'&ifcheck=1'; ?>" class="shield J_tag_shield" title="取消屏蔽">取消屏蔽</a>
											<?php  } 
  } ?>
										<div class="content">
											<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>" class="name J_user_card_show" data-uid="<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?></a>：
											<em><a href="<?php echo Wekit::app()->baseUrl,'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']); ?>" class="title"><?php echo htmlspecialchars($value['subject'], ENT_QUOTES, 'UTF-8');?></a></em><br>
											<div class="descrip"><?php echo htmlspecialchars(Pw::substrs(Pw::stripWindCode($value['content'],true),125), ENT_QUOTES, 'UTF-8');?></div>
										</div>
										<div class="info">
											<span class="time"><?php echo htmlspecialchars($value['created_time_auto'], ENT_QUOTES, 'UTF-8');?></span>&nbsp;来自版块&nbsp;-&nbsp;<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=thread&fid=', rawurlencode($value['fid']); ?>"><?php echo $value['forum_name'];?></a>
											<?php  if($relatedTags[$value['tid']]){ 
  foreach($relatedTags[$value['tid']] as $k=>$v){ if ($k==$key) continue; ?>
												<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>"><?php echo htmlspecialchars($v['tag_name'], ENT_QUOTES, 'UTF-8');?></a>
												<?php  } 
  } ?>
										</div>
									</dd>
...
<div class="box_wrap">
	...
	<div class="tag_hot_list">
		<ul>
			...
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>" class="title"><?php echo htmlspecialchars($v['tag_name'], ENT_QUOTES, 'UTF-8');?></a><em class="num"><?php echo htmlspecialchars($v['content_count'], ENT_QUOTES, 'UTF-8');?></em></li>
			<?php  } if (count($hotTagList) > 9) { ?>
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag'; ?>#tag_more_list" class="title">查看更多</a></li>
			<?php  } ?>
		</ul>
	</div>
</div>
...
<div class="box_wrap">
	...
	<div class="side_cate_list">
		...
		<ul id="J_side_my_tags">
			...
			<li><a data-id="<?php echo htmlspecialchars($v['tag_id'], ENT_QUOTES, 'UTF-8');?>" title="取消关注该话题" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&type=del&id=', rawurlencode($v['tag_id']); ?>" class="icon_del J_tag_del">删除</a><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view&name=', rawurlencode($v['tag_name']); ?>" class="title"><?php echo htmlspecialchars($v['tag_name'], ENT_QUOTES, 'UTF-8');?><em>(<?php echo htmlspecialchars($v['content_count'], ENT_QUOTES, 'UTF-8');?>)</em></a></li>
			<?php  } if ($myTags['step']) { ?>
			<li><a id="J_tag_more" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attentionlist'; ?>" data-viewurl="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=view'; ?>" data-delurl="<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&type=del'; ?>" class="title" data-step="<?php echo htmlspecialchars($myTags['step'], ENT_QUOTES, 'UTF-8');?>">查看更多</a></li>
			<?php  } ?>
		</ul>
		<?php  } else { ?>
		<div style="padding:20px 0;"><div class="not_content_mini">你还没关注任何话题</div></div>
		<?php  } ?>
	</div>
</div>
<?php  } ?>
...
<script>
var URL_ADD = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&type=add'; ?>',	//关注
	URL_DEL = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=tag&a=attention&type=del'; ?>';	//取消关注
