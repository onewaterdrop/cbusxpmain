...
<link href="<?php echo Wekit::app()->themes.'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/announce.css?v=<?php echo htmlspecialchars(NEXT_RELEASE, ENT_QUOTES, 'UTF-8');?>" rel="stylesheet" />
</head>
<body>
<div class="wrap">
...
<div class="tac"> </div>
	<div class="main_wrap">
		<div class="bread_crumb">
			<a href="<?php echo Wekit::app()->baseUrl; ?>" class="home" title="<?php echo Wekit::C("site", "info.name"); ?>">首页</a><em>&gt;</em><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=announce'; ?>">公告</a>
		</div>
		<div class="main cc">
			<div class="main_body">
  ...
  if($aid == $value['aid'])$classValue = 'class=current'; ?>
						<dl <?php echo htmlspecialchars($classValue, ENT_QUOTES, 'UTF-8');?>>
							<dt>
							<?php  $subject = null; 
										if($value['typeid']){
  ...
  if($value['typeid']){ ?>
							<span class="subject"><a href="<?php echo htmlspecialchars($value['url'], ENT_QUOTES, 'UTF-8');?>" target="_blank"><?php echo htmlspecialchars($value['subject'], ENT_QUOTES, 'UTF-8');?></a></span>
							<?php  }else{ ?>
							 <span class="subject"><?php echo htmlspecialchars($value['subject'], ENT_QUOTES, 'UTF-8');?></span>
							<?php  } ?>
							<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>" class="name"><?php echo htmlspecialchars($value['username'], ENT_QUOTES, 'UTF-8');?></a>
							<?php  
							$start_date = Pw::time2str($value['start_date'], 'Y-m-d'); 
							$end_date = Pw::time2str($value['end_date'], 'Y-m-d'); 
							?>
							<span class="time"><?php echo htmlspecialchars($start_date, ENT_QUOTES, 'UTF-8');?> 至 <?php echo htmlspecialchars($end_date, ENT_QUOTES, 'UTF-8');?></span></dt>
							<?php  $content = $value['typeid'] ? null : $value['content']; ?>
							<dd>
							<?php echo $content;?>
							</dd>
						</dl>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=announce&page=', rawurlencode($_page_i); 
