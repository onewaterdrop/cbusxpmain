...
<script>
	...
	*/
	...
	var IMAGE_CONFIG = {
		...
		uploadUrl: "<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=upload&a=dorun&_json=1'; ?>",//上传地址
	...
	var ATTACH_CONFIG = {
		...
		uploadUrl : "<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=upload&a=dorun&_json=1'; ?>",//上传地址
		modifyUrl : "<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=upload&a=replace&_json=1'; ?>",//修改地址
		deleteUrl : '<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=attach&a=delete&_json=1'; ?>',
    ...
    //表情数据的地址
    var EMOTION_URL = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=emotion&type=bbs'; ?>';
    //at好友数据地址
    var AT_URL = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=remind'; ?>';
    //at弹窗的地址
    //var AT_DIALOG_URL = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=remind&a=friend'; ?>';
    var AT_USER_SPACE = '<?php echo Wekit::app()->baseUrl,'/','index.php?m=space'; ?>?uid=';
   //应用插件配置
	var editorApp = {
		root: '<?php echo htmlspecialchars(Wind::getComponent('response')->getData('G','url','extres'), ENT_QUOTES, 'UTF-8');?>/',
		items: <?php echo WindJson::encode($editor_app_config, 'UTF-8');?>
	};
    /*
	* 引用编辑器js并初始化编辑器
	*/
	Wind.ready('global.js', function() {
		Wind.use('jquery', 'windeditor',function() {
			...
			$('#J_wind_editor').windeditor({
				...
				iframeCss	: ['<?php echo Wekit::app()->themes.'/site/default/css'.Wekit::getGlobal('theme','debug'); ?>/editor_content.css?v=<?php echo htmlspecialchars(Wind::getComponent('response')->getData('G','c','version'), ENT_QUOTES, 'UTF-8');?>'],//为了更好的可见即所得，加载阅读页的css文件
				onInit		: function () {
