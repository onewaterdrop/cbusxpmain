...
<!--默认风格列表-->
	<div class="nav">
		<ul class="cc">
			...
			<li class="<?php echo htmlspecialchars(Pw::isCurrent($key==$type), ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?type=', rawurlencode($key),'&m=appcenter&c=style'; ?>"><?php echo htmlspecialchars($value[0], ENT_QUOTES, 'UTF-8');?></a></li>
			<?php  } ?>
	...
	<div class="design_page">
		...
		foreach($styles as $id => $v){?>
			...
			if($logo && (strpos($logo,'http://')===false)) {
				$args = array(Wekit::app()->themes, $addons[$type][1], $v['alias'], $logo);
				$logo = implode('/', $args);
			}
			?>
 ...
 echo htmlspecialchars($v['author_name'] ? $v['author_name'] : '请叫我雷锋', ENT_QUOTES, 'UTF-8');
  ...
  if($v['iscurrent'] == 1){ ?>
					...
					<?php  }else{ ?>
					<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?styleid=', rawurlencode($id),'&m=appcenter&c=style&a=default'; ?>" class="J_ajax_refresh">设为默认</a><a data-msg="确定要卸载吗？" href="<?php echo Wekit::app()->baseUrl,'/','admin.php?styleid=', rawurlencode($id),'&m=appcenter&c=style&a=uninstall'; ?>" class="J_ajax_del">卸载</a>
					<?php  }if($type == 'site'){ ?>
					<!-- <a target="_blank" href="<?php echo Wekit::app()->baseUrl,'/','admin.php?styleid=', rawurlencode($id),'&m=appcenter&c=style&a=preview'; ?>">预览</a> -->
					<?php  }} else { ?>
					<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?styleid=', rawurlencode($id),'&m=appcenter&c=style&a=uninstall'; ?>" class="J_ajax_del">卸载</a>
					<?php  } if(Wekit::C('site', 'debug') && $type != 'portal'){ ?>
					<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?type=', rawurlencode($type),'&alias=', rawurlencode($v['alias']),'&m=appcenter&c=style&a=export'; ?>">导出</a>
					<?php  } ?>
					<a id="app_update_<?php echo htmlspecialchars($v['app_id'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wekit::app()->baseUrl,'/','admin.php?app_id=', rawurlencode($v['app_id']),'&m=appcenter&c=app&a=upgrade'; ?>" class="J_ajax_upgrade" data-msg="确定要升级吗？升级将覆盖原先应用" style="display:none">升级</a>
				</div>
			</li>
			<?php }?>
		</ul>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?type=', rawurlencode($type),'&page=', rawurlencode($_page_i),'&m=appcenter&c=style'; 
