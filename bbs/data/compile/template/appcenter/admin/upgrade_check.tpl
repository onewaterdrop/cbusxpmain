...
<div class="wrap">
	...
	<?php  if(isset($connect_fail)){ ?>
		<div class="tips">
		<?php  if($connect_fail){ 
 echo htmlspecialchars($connect_fail, ENT_QUOTES, 'UTF-8');
  } else { ?>
		无法连接云平台，请尝试申请云服务。或选择手动升级。
		<?php  } ?>
		</div>
	<?php  } else { 
  if($result){ ?>
				...
				<tbody >
					<?php  foreach($result as $v){ ?>
						<tr>
							...
							<td><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?version=', rawurlencode($v['version']),'&m=appcenter&c=upgrade&a=select'; ?>" class="btn J_ajax_confirm" data-msg="确定已关闭网站，并建议备份程序和数据库。">立即升级</a></td>
							<td></td>
						</tr>
					<?php  } ?>
				</tbody>
		...
		<?php  } else { ?>
		<div class="tips"><b>恭喜，您使用的是最新版本！</b></div>
		<?php  } 
  } ?>
	</div>
</div>
