...
<div class="wrap">
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=appcenter&c=app'; ?>">已安装</a></li>
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=appcenter&c=app&a=install'; ?>">本地安装</a></li>
			<?php  if(Wekit::load('APPS:appcenter.service.srv.PwDebugApplication')->inDevMode1()){ ?>
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=appcenter&c=develop'; ?>">开发助手</a></li>
			<?php  } ?>
		</ul>
	</div>
...
<?php }else{ ?>
	<div class="mb10 cc">
	<form action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=appcenter&c=app&a=search'; ?>" method="POST">
		<span class="fl mr20">
...
?>
			<tr>
				<td><img src="<?php echo htmlspecialchars($app['logo'], ENT_QUOTES, 'UTF-8');?>" onerror="this.onerror=null;this.src='<?php echo htmlspecialchars(Wind::getComponent('response')->getData('G','url','images'), ENT_QUOTES, 'UTF-8');?>/admin/yy.png'" align="absmiddle" class="app_icon" alt="<?php echo htmlspecialchars($app['name'], ENT_QUOTES, 'UTF-8');?>" width="80" height="80"></td>
				<td valign="top">
					<?php  if($app['status'] & 1){ ?>
						<h3 class="mb5 f12"><a target="_blank" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=app&app=', rawurlencode($app['alias']); ?>"><?php echo htmlspecialchars($app['name'], ENT_QUOTES, 'UTF-8');?></a></h3>
					<?php  }else{ ?>
				...
				<td>
				<?php  if($app['status'] & 2){ ?>
				<a id="app_set_<?php echo htmlspecialchars($app['app_id'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wekit::app()->baseUrl,'/','admin.php?app=', rawurlencode($app['alias']),'&m=app&c=manage'; ?>" class="btn btn_submit btn_big mr5">设置</a>
				...
				<?php  }else{ ?>
				<button title="无需设置" class="btn btn_submit btn_big mr5 disabled" style="margin:0;" disabled>设置</button><?php  } ?><button data-act="卸载" data-msg="卸载将清除所有数据，确定要卸载吗？" data-action="<?php echo Wekit::app()->baseUrl,'/','admin.php?app_id=', rawurlencode($app['app_id']),'&m=appcenter&c=app&a=uninstall'; ?>" class="J_ajax_upgrade btn_big btn">卸载</button>
				<button data-act="升级" data-msg="确定要升级吗？升级将覆盖当前应用" id="app_update_<?php echo htmlspecialchars($app['app_id'], ENT_QUOTES, 'UTF-8');?>" data-action="<?php echo Wekit::app()->baseUrl,'/','admin.php?app_id=', rawurlencode($app['app_id']),'&m=appcenter&c=app&a=upgrade'; ?>" class="J_ajax_upgrade btn btn_big btn_error" style="display:none">升级</button>
				<?php  if($app['status'] & 4 && Wekit::load('APPS:appcenter.service.srv.PwDebugApplication')->inDevMode1()){ ?>
				<div><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?alias=', rawurlencode($app['alias']),'&m=appcenter&c=develop&a=edit'; ?>" class="btn btn_big mr5">设计</a>
				<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?alias=', rawurlencode($app['alias']),'&m=appcenter&c=app&a=export'; ?>" class="btn btn_big">导出</a></div>
				<?php  } ?>
				</td>
			</tr>
<?php  } ?>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?keyword=', rawurlencode($keyword),'&page=', rawurlencode($_page_i),'&m=appcenter&c=app&a=search'; 
...
<?php  if ($__tplPageCurrent > $_page_min) { 
	$_page_i = $__tplPageCurrent-1;
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?page=', rawurlencode($_page_i),'&m=appcenter&c=app'; 
 echo htmlspecialchars(array() ? '&' . http_build_query(array()) : '', ENT_QUOTES, 'UTF-8');?>" class="pages_pre J_pages_pre">&laquo;&nbsp;上一页</a>
	<?php  if ($_page_min > 1) { 
		$_page_i = 1;		
	?>
...
<script>
var url = '<?php echo Wekit::app()->baseUrl,'/','admin.php?m=appcenter&c=app&a=refresh'; ?>',
