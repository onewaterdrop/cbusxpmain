...
<div class="wrap J_check_wrap">
	<div class="nav">
		<ul class="cc">
			<li class="current"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=article'; ?>">帖子管理</a></li>
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=article&a=replylist'; ?>">回复管理</a></li>
		</ul>
	</div>
	...
	<?php  $checkedDigest = $args['digest'] ? 'checked=checked' : '' ?>
	<form method="post" action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=article&a=searchthread'; ?>">
	<div class="search_type cc mb10">
		<div class="ul_wrap">
			<ul class="cc">
				<li>
					<label>关键字：</label><input name="keyword" value="<?php echo htmlspecialchars($args['keyword'], ENT_QUOTES, 'UTF-8');?>" type="text" class="input length_3" placeholder="支持帖子标题和正文搜索">
				</li>
				<li>
					<label>作者：</label><input name="created_username" value="<?php echo htmlspecialchars($args['created_username'], ENT_QUOTES, 'UTF-8');?>" type="text" class="input length_3">
				</li>
				<li>
					<label>所属版块：</label><select name="fid" class="select_3"><option value="0">所有版块</option><?php echo $option_html;?></select>
				</li>
				<li>
		...
		<div class="btn_side">
			<button class="btn mr20" type="submit">搜索</button><a class="mr20 J_dialog" href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=article&a=threadadvanced'; ?>" title="高级搜索">高级搜索</a><label class="w" style="display:none"><input type="checkbox" name="digest" value="1" <?php echo htmlspecialchars($checkedDigest, ENT_QUOTES, 'UTF-8');?>>过滤精华</label>
		</div>
	</div>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
	
	<form class="J_ajaxForm" action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=article&a=deletethread'; ?>" method="post">
		<div class="table_list">
			<table width="100%">
				...
				$v['content'] = Pw::substrs($v['subject'], 20);
				$_forumSubject = strip_tags($forumList[$v['fid']]['name']);
				?>
				<tr>
					<td><input type="checkbox" class="J_check" data-yid="J_check_y" data-xid="J_check_x" name="tids[]" value="<?php echo htmlspecialchars($v['tid'], ENT_QUOTES, 'UTF-8');?>"></td>
					<td><a href="<?php echo Wekit::app()->baseUrl,'/','read.php?tid=', rawurlencode($v['tid']),'&fid=', rawurlencode($v['fid']); ?>" target="_blank"><?php echo htmlspecialchars($v['subject'], ENT_QUOTES, 'UTF-8');?></a></td>
					...
					<td><?php echo htmlspecialchars($v['replies'], ENT_QUOTES, 'UTF-8');?>/<?php echo htmlspecialchars($v['hits'], ENT_QUOTES, 'UTF-8');?></td>
				</tr>
				<?php  } 
  } else { ?>
					<tbody>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?page=', rawurlencode($_page_i),'&m=bbs&c=article&a=searchthread'; 
		...
		<div class="btn_wrap">
			<div class="btn_wrap_pd">
				...
				<?php  
				 ...
				 ?>
					<a href="<?php echo htmlspecialchars(WindUrlHelper::createUrl('bbs/article/searchthread?perpage=20', $args), ENT_QUOTES, 'UTF-8');?>">20</a><span>|</span>
					<a href="<?php echo htmlspecialchars(WindUrlHelper::createUrl('bbs/article/searchthread?perpage=50', $args), ENT_QUOTES, 'UTF-8');?>">50</a><span>|</span>
					<a href="<?php echo htmlspecialchars(WindUrlHelper::createUrl('bbs/article/searchthread?perpage=100', $args), ENT_QUOTES, 'UTF-8');?>">100</a>
				</div>
				<label class="mr20"><input type="checkbox" class="J_check_all" data-direction="y" data-checklist="J_check_y">全选</label>
				<button class="btn J_ajax_submit_btn" type="submit">删除</button>
				<button class="btn btn_submit J_ajax_submit_btn" type="submit" data-action="<?php echo Wekit::app()->baseUrl,'/','admin.php?isDeductCredit=1&m=bbs&c=article&a=deletethread'; ?>">删除并扣除积分</button>
			</div>
		</div>
		<?php  } ?>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
