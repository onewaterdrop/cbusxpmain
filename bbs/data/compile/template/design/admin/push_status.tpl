...
?>
	<div class="nav">
		<ul class="cc">
			<li><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=design&c=push'; ?>">显示中数据</a></li>
			<li  class="<?php echo htmlspecialchars($check, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?status=1&m=design&c=push&a=status'; ?>">待审核数据</a></li>
			<li class="<?php echo htmlspecialchars($shield, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?status=2&m=design&c=push&a=status'; ?>">推送数据</a></li>
	...
	<form method="post"  action="<?php echo Wekit::app()->baseUrl,'/','admin.php?status=', rawurlencode($status),'&m=design&c=push&a=status'; ?>" >
	<div class="search_type cc mb10">
		...
		<input type="hidden"  name="check" value="<?php echo htmlspecialchars($check, ENT_QUOTES, 'UTF-8');?>">
	</div>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
	<form class="J_ajaxForm" method="post"  action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=design&c=push&a=batchcheck'; ?>" >
  ...
  foreach ($list as $v):
				 $isExpired = false;
				if ($v['end_time'] > 0 && $v['end_time'] < Pw::getTime()) $isExpired = true;
			?>
			<tr>
				<td><input data-yid="J_check_y" data-xid="J_check_x" class="J_check" type="checkbox" name="pushids[]" value="<?php echo htmlspecialchars($v['push_id'], ENT_QUOTES, 'UTF-8');?>"></td>
				<td><?php  if ($isExpired): ?><span style="color:#990000">[已过期]</span><?php  endif; ?><a href="<?php echo htmlspecialchars($v['url'], ENT_QUOTES, 'UTF-8');?>" target="_blank"><?php echo htmlspecialchars($v['title'], ENT_QUOTES, 'UTF-8');?></a></td>
				<td><?php echo htmlspecialchars($modules[$v['module_id']]['module_name'], ENT_QUOTES, 'UTF-8');?></td>
				<td><?php echo htmlspecialchars($users[$v['created_userid']]['username'], ENT_QUOTES, 'UTF-8');?></td>
				<?php  if ($status == 1): ?>
				...
				<?php  endif; ?>
				<td>
					<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?pushid=', rawurlencode($v['push_id']),'&m=design&c=push&a=delpush'; ?>" class="mr10 J_ajax_del">[删除]</a>
					<?php  if ($status == 1): ?>
					<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?pushid=', rawurlencode($v['push_id']),'&m=design&c=push&a=dopush'; ?>" class="mr10 J_ajax_refresh">[通过]</a>
					<?php  endif; ?>
				</td>
			</tr>
		<?php  endforeach; 
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?page=', rawurlencode($_page_i),'&m=design&c=push&a=status'; 
...
<?php } ?>
	...
	<div class="btn_wrap_pd">
		...
		<?php  endif; ?>
		<button data-subcheck="true" data-msg="确定要删除选中内容?" class="btn btn_submit J_ajax_submit_btn" type="submit" data-action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=design&c=push&a=batchdelete'; ?>">删除</button>
	</div>
	</div>
	<?php  } ?>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>	
