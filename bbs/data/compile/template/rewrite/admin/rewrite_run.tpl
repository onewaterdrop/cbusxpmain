...
<div class="prompt_text">
	<ul>
		...
		<li>如果url格式中包含{fname}参数，则必须在<a href="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=bbs&c=setforum'; ?>">【版块编辑】</a>中设置版块的个性域名后才能正常访问。</li>
	</ul>
</div>
<form method="post" class="J_ajaxForm" action="<?php echo Wekit::app()->baseUrl,'/','admin.php?m=rewrite&c=rewrite&a=doModify'; ?>">
...
<table width="100%">
		...
		<tr>
			...
			<td><input name="isopen[<?php echo htmlspecialchars($k, ENT_QUOTES, 'UTF-8');?>]" type="checkbox" value="1" <?php echo htmlspecialchars(Pw::ifcheck($rewrite["isopen.$k"]), ENT_QUOTES, 'UTF-8');?>></td>
		</tr>
		<?php  } ?>
	</table>
</div>
