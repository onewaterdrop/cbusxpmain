...
if($job=="config"&&$Apower[center_config])
{
	...
	$yzimg_letter_differen[intval($webdb[yzimg_letter_differen])]=' checked ';
	$yzimg_simple[intval($webdb[yzimg_simple])]=' checked ';
...
elseif($job=="user_reg"&&$Apower[center_config])
{
...
elseif($job=='cache'&&$Apower[center_cache])
{
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/center/menu.htm");
...
{
	if(is_file(dirname(__FILE__)."/"."../index.htm")){
		@unlink(dirname(__FILE__)."/"."../index.htm.bak");
		@rename(dirname(__FILE__)."/"."../index.htm","../index.htm.bak");
	}
	if($returnto){
		jump("执行完毕",$returnto);
	}
...
elseif($job=="map")
{
	...
	@extract($db->get_one("SELECT COUNT(id) AS Article FROM {$pre}fenlei_db"));
	@extract($db->get_one("SELECT COUNT(cid) AS Comment FROM {$pre}fenlei_comments"));
	@extract($db->get_one("SELECT COUNT(id) AS Article0 FROM {$pre}fenlei_content WHERE yz=0"));
	@extract($db->get_one("SELECT COUNT(cid) AS Comment0 FROM {$pre}fenlei_comments WHERE yz=0"));
	@extract($db->get_one("SELECT COUNT(id) AS Guestbook FROM {$pre}fenlei_report"));
	@extract($db->get_one("SELECT COUNT($TB[uid]) AS Member FROM $TB[table]"));

	$Member = $userDB->total_num();
	//require("head.php");

	if(!strstr(preg_replace("/http:\/\/(www\.|)(.*)/is","\\2",$WEBURL),preg_replace("/http:\/\/(www\.|)(.*)/is","\\2",$webdb[www_url]))){
