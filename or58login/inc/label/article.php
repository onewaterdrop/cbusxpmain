...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,A.aid AS id,R.content FROM {$pre}article$erp A LEFT JOIN {$pre}reply R ON A.aid=R.aid $SQL AND A.ispic=1 AND R.topic=1 ORDER BY $order $asc LIMIT 1 ";
		}else{
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,A.aid AS id,R.content FROM {$pre}article$erp A LEFT JOIN {$pre}reply R ON A.aid=R.aid  $SQL $SQLPIC AND R.topic=1 ORDER BY $order $asc LIMIT $rows ";
	}else{
