	...
	foreach($filedb AS $word=>$array)
	{
		if($Apower[$array[power]]||is_numeric($array[power]))
		{
			if($array[power]=="label_index"){
				$show.="<tr><td onmouseover=\"this.className='leftA'\" onmouseout=\"this.className='leftB'\"><A HREF='$pathurl$array[link]' target=main>$word</A> | <A HREF='../index.php?&ch=1&MakeIndex=1' target='_blank' onclick=\"return confirm('你确实要把首页生成静态吗?生成静态后,如有更改其它相关设置,需要重新点击生成一次静态.才可以看到效果.');\">主页静态</A></td></tr>";
			}else{
